...
void PLCD_voidWrite8Bits(LCD_t* LCD, u8 Byte){
	for(u8 i = 0 ; i < 8 ; i++){
		MGPIO_voidSetPinValue(LCD->DataPorts[i],LCD->DataPins[i],GET_BIT(Byte,i));
	}
	PLCD_voidEnable(LCD);
}
...
void PLCD_voidWrite4Bits(LCD_t* LCD, u8 HalfByte){
	for(u8 i = 0 ; i < 4 ; i++){
		MGPIO_voidSetPinValue(LCD->DataPorts[i],LCD->DataPins[i],GET_BIT(HalfByte,i));
	}
	PLCD_voidEnable(LCD);
}
...
void PLCD_voidWaitForBusyFlag(LCD_t* LCD){
	...
	Local_u8BusyFlag = MGPIO_u8GetPinValue(LCD->ControlPorts[Local_u8DB7],LCD->ControlPins[Local_u8DB7]);
...
void HLCD_voidInit(LCD_t* LCD){
	...
	for(u8 i = 0; i< 3; i++){
		MGPIO_voidSetPinDirection(LCD->ControlPorts[i],LCD->ControlPins[i],GPIO_DIR_OUTPUT_PUSH_PULL);
	}
